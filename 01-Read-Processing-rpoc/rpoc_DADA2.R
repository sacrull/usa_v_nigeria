...
#Set up paths
rawpath <- "/home/suzanne/usa_nigeria/rpoc/usa_nigeria_rawfastq"
...
track
# write to file
write.table(data.frame("row_names"=rownames(track),track),"read_retention_rpoc.txt", row.names=FALSE, quote=F, sep="\t")
uniquesToFasta(seqtab.nochim, "rep_set.fa")
# fix ASV names 
system("awk '/^>/{print \">rASV\" ++i; next}{print}' < rep_set.fa > rep_set_fix.fa")
system("mv rep_set_fix.fa rep_set_rpoc.fa")
...
write.table(data.frame("row_names"=rownames(seqtab.nochim),seqtab.nochim),"sequence_table_rpoc.merged.txt", row.names=FALSE, quote=F, sep="\t")
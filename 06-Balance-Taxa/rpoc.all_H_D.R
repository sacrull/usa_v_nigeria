...
rpoc.dat <- prune_samples(sample_sums(rpoc.pd) > 4000, rpoc.pd) #remove less than 2000 reads
rpoc.dat <- subset_samples(rpoc.dat , hiv_status == "HUU" | hiv_status == "NAN")
#rpoc.dat <- subset_taxa(rpoc.dat, V2!="Eukaryote_unknown" & V3!="Fungi_unknown")
#usa
...
map <- sample_data(glom)
#get taxonomy of ASV
taxa = as(tax_table(rpoc.pd), "matrix")
...
#nigeria
# collapse data to roughly species level to minimize high sparsity
rpoc.geo <- subset_samples(rpoc.dat, Geog_loc == "Nigeria" & hiv_status == "HUU")
rpoc.cd <- subset_samples(rpoc.geo, tooth_health == "H" | tooth_health == "D")
glom <- tax_glom(rpoc.cd, "V8")
# remove any taxa with fewer than 50 counts and in at least 10% of samples post merging
#glom <- filter_taxa(glom, function(x) sum(x > 50) > (0.10*length(x)), TRUE)
...
#H USA vs Nigeria
# collapse data to roughly species level to minimize high sparsity
rpoc.geo <- subset_samples(rpoc.dat, hiv_status == "NAN" | hiv_status == "HUU")
rpoc.cd <- subset_samples(rpoc.geo, tooth_health == "H")
glom <- tax_glom(rpoc.cd, "V8")
# remove any taxa with fewer than 50 counts and in at least 10% of samples post merging
#glom <- filter_taxa(glom, function(x) sum(x > 50) > (0.10*length(x)), TRUE)
# pull data
...
# collapse data to roughly species level to minimize high sparsity
rpoc.geo <- subset_samples(rpoc.dat, hiv_status == "NAN" | hiv_status == "HUU")
rpoc.cd <- subset_samples(rpoc.geo, tooth_health == "D")
glom <- tax_glom(rpoc.cd, "V8")
# remove any taxa with fewer than 50 counts and in at least 10% of samples post merging
#glom <- filter_taxa(glom, function(x) sum(x > 50) > (0.10*length(x)), TRUE)
# pull data

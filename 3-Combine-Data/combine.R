...
its.pd <- prune_samples(!(sample_names(its.pd) %in% to_remove_its), its.pd)
...
rpoc.pd <- prune_samples(!(sample_names(rpoc.pd) %in% to_remove_rpoc), rpoc.pd)
rpoc.dat <- subset_samples(rpoc.pd , hiv_status == "HUU" | hiv_status == "NAN")

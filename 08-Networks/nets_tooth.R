...
set.seed=123456
# USA rpoc
rpoc.usa <- subset_samples(rpoc.dat, Geog_loc == "USA" & tooth_health == "D")
...
# USA its
its.usa <- subset_samples(its.dat, Geog_loc == "USA" & tooth_health == "D")
glom.its.usa <- tax_glom(its.usa, "V8")
# filter low abundance taxa to simplify network building step (seen at least 3 times in 10% of samples)
its.usa<- filter_taxa(glom.its.usa, function(x) sum(x > 3) > (0.05*length(x)), TRUE)

#Niger rpoc
rpoc.nigeria <- subset_samples(rpoc.dat, Geog_loc == "Nigeria" & tooth_health == "D")
...
# Niger ITS
its.nigeria <- subset_samples(its.dat, Geog_loc == "Nigeria" & tooth_health == "D")
glom.its.nigeria <- tax_glom(its.nigeria, "V8")
# filter low abundance taxa to simplify network building step (seen at least 3 times in 10% of samples)
its.nigeria<- filter_taxa(glom.its.nigeria, function(x) sum(x > 3) > (0.10*length(x)), TRUE)
#network generation
...
save.image("cross_nets_D.RData")

load("cross_nets_D.RData")
#usa.cross
...
#edge width
import_taxa2 = c("Debaryomyces_prosopidis", "Candida_tropicalis", "Candida_albicans", "Candida_unknown","Rhodotorula_mucilaginosa",
...
#graph
pdf("usa_positive_H.pdf", height = 100, width = 100)
